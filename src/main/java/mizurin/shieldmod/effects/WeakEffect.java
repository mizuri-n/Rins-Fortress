...
public class WeakEffect extends Effect {
	public final Random random = new Random();

	public WeakEffect(String nameKey, String id, List<Modifier<?>> modifiers, EffectTimeType effectTimeType, int maxStack) {
	...
	public <T> void expired(EffectStack effectStack, EffectContainer<T> effectContainer) {
		effectContainer.remove(ShieldEffects.weaknessEffect);
		EffectStack newStack = new EffectStack((IHasEffects) effectContainer.getParent(), ShieldEffects.weaknessEffect, effectStack.getAmount() - 1);
	...
	public <T> void tick(EffectStack effectStack, EffectContainer<T> effectContainer) {
		...
		if (mob.world == null) {
			ShieldMod.LOGGER.warn("WeaknessEffect is not applied cause the world is null");
			return;
		}
		if(mob.tickCount % 60 == 0 && mob instanceof MobZombie){
